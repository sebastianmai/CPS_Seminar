...
if __name__ == '__main__':
    ...
    plt.subplot(2, 1, 1)
    plt.plot(res_yaw, label='Yaw')
    plt.plot(res_error, label='Error')
    plt.plot(res_thruster_pos, label='Thruster Position Command')
    plt.title("Results")
    plt.xlabel("Time")
    plt.ylabel("Output values")
    plt.legend()

    # Plot 2: Boat Position
    plt.subplot(2, 1, 2)
    boat_pos = list(zip(*boat_pos))
    plt.plot(boat_pos[0], boat_pos[1],)
